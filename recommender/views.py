...
import json
import csv
# Import new University model and old Course model
from .models import Assessment, Course, University 

# Note on Lazy Loading: Heavy imports like pandas, joblib, matplotlib are moved inside the functions that need them.

# --- CRITICAL: EXPANDED AND REFINED COURSE_PERSONAS DICTIONARY (87 Courses) ---
COURSE_PERSONAS = {
    # -----------------------------------------------------
    ...
    'Veterinary or Animal Sciences': {'key_traits': ['interest_nature', 'interest_helping', 'ability_practical'], 'label': 'Animal Care Specialist', 'profile': {'interest_nature': 5, 'interest_helping': 5, 'ability_practical': 4, 'interest_research': 3, 'ability_teamwork': 4}},
    'Zoology / Systematics & Ecology': {'key_traits': ['interest_nature', 'interest_research', 'ability_logic'], 'label': 'Animal Classifier', 'profile': {'interest_nature': 5, 'interest_research': 5, 'ability_logic': 4, 'ability_practical': 3, 'interest_helping': 3}},
    # Placeholder for the rest of the 87 courses
    # ... (Add remaining courses here using the structure above)
}

def get_in_depth_insights(course_name, user_ratings):
    # This function uses the COURSE_PERSONAS to generate personalized text and chart data.
    ...
    # Use a combined set of the key traits and the top 5 traits from the ideal profile for charting
    ...
    for trait in chart_traits:
        ...
        if trait_name:
            ...
            # Generate insights only for key_traits
...
# --- Student Views (Dashboard, Courses, About, Assessment - Remain the same) ---
...
# --- UNIVERSITY INFO VIEW (NEW FEATURE) ---
# In recommender/views.py, REPLACE the existing university_info_view:

# --- UNIVERSITY INFO VIEW (FIXED ARGUMENT AND LOOKUP) ---
def university_info_view(request, uni_slug):
    
    # The slug comes in clean (e.g., 'university-of-the-philippines-up')
    name_parts = uni_slug.replace('-', ' ').title().split()
    
    try:
        # We search by the first word (University) for robustness, 
        # or join the full slug name for a more precise match
        search_name = ' '.join(name_parts)
        
        # This lookup is complex due to varied names (e.g., 'University of the East (UE)')
        # We'll use a robust filter that can match 'University of the East' even if the slug is simplified.
        # Use Q objects for OR logic to maximize matching probability:
        ...
        # Fallback if no specific university is found
        raise Http404("University information not found.")
...
# --- UPDATED RECOMMENDATION VIEW (Hybrid Two-Stage Logic with Weighting and Uni Lookup) ---
def recommendation_view(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Save raw assessment data (using new fields)
            new_assessment = Assessment.objects.create(
                ...
                interest_research=form_data.get('interest_research', 0), interest_arts=form_data.get('interest_arts', 0),
                interest_policy=form_data.get('interest_policy', 0), interest_design=form_data.get('interest_design', 0),
                interest_tech=form_data.get('interest_tech', 0), interest_building=form_data.get('interest_building', 0), 
                interest_nature=form_data.get('interest_nature', 0), ability_spatial=form_data.get('ability_spatial', 0),
                interest_detail=form_data.get('interest_detail', 0), interest_leading=form_data.get('interest_leading', 0), 
                interest_helping=form_data.get('interest_helping', 0), ability_teamwork=form_data.get('ability_teamwork', 0),
            ...
            # --- STAGE 1: Predict the Field Category (Logic remains the same) ---
            ...
            for column, encoder in encoders.items():
                if column in df_user.columns:
                    ...
                    try: encoded_value = encoder.transform(value_to_transform)[0]; df_user.at[0, column] = encoded_value
            ...
            # --- STAGE 2: Rank Courses with TRAIT WEIGHTING ---
            ...
            for course_obj in all_qualifying_courses:
                ...
                for trait, ideal_score in persona.get('profile', {}).items():
                    ...
                    # 1. Calculate Base Match Score (5 - absolute difference)
                    score_increment = 5 - abs(user_score - ideal_score) 
                    
                    # 2. Apply Weighting: Core Traits get Triple Weight (3x)
                    weight = 1
                    if trait in key_traits:
                        weight = 3 
            ...
            for i, (course, data) in enumerate(top_3_ranked_courses):
                ...
                recommendations.append({
                    ...
                    'insights': insights,
                    'universities': uni_list # PASS NEW DATA
                })
            
            # Save the final top 3 courses (Logic remains the same)
...
# --- Admin/Auth Views (The rest of the file remains largely the same) ---
# ... (admin_password_reset_request_view, login_view, logout_view, admin_dashboard_view, export_analytics_view, delete_all_assessments_view remain the same)

# Course Management Views (course_list_view, course_create_view, etc. - Remain the same but must handle new field_category)
# ... (course_list_view, course_create_view, course_update_view, course_delete_view remain the same)
# ... (export_analytics_pdf_view remains the same)
...
def generate_feedback_data_view(request):
    import pandas as pd # Re-import here for lazy loading
    assessments_with_feedback = Assessment.objects.filter(feedback_submitted=True)
    new_training_data = []
    
    # We need the field_category for the new training data rows
    ...
    for assessment in assessments_with_feedback:
        ...
        # Helper function to append feedback data
        def append_feedback(course_name, rating):
            if course_name:
                new_row = base_data.copy()
                new_row['course'] = course_name
                # Add the critical 'field_category' based on the map
...
# --- Admin/Auth Views (login_view, admin__view, etc. - Remain the same) ---
...
def export_analytics_view(request):
    import csv # Lazy import for this function
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="assessment_analytics.csv"'
    writer = csv.writer(response)
    # Added field_category to export for richer analytics
    writer.writerow(['ID', 'Name', 'School', 'SHS Strand', 'TVL Strand', 'Rec 1', 'Rec 2', 'Rec 3', 'Field Rec 1', 'Field Rec 2', 'Field Rec 3', 'Timestamp'])
    # Note: We can't easily retrieve the Field Recs from the Assessment model as they weren't saved, but we keep the row structure for future proofing.
...
def course_create_view(request):
    from .models import FIELD_CHOICES # Import choices for the form
    if request.method == 'POST':
        Course.objects.create(
            name=request.POST.get('name'), 
            description=request.POST.get('description'), 
            icon=request.POST.get('icon'),
            field_category=request.POST.get('field_category') # Save the new field
        )
...
def export_analytics_pdf_view(request):
    # This view remains largely the same, but imports are grouped at the top of the function for lazy loading.
    import io
